...
def rtm(token, queue, workspace, workspace_token):
    ...
    for x in slack_api_members:
        ...
        query_res = Student.get_by_user_workspace(x['id'], workspace)
        if query_res:
            member = query_res
        else:
            member = Student(name, x['profile']['real_name'], x['id'], workspace)
            member.add()
            ...
            while True:
                ...
                if len(res)>0:
                    ...
                    if res[0].get('channel', False) == 'D9J7J4GJG' and res[0].get('text', False)=='help':
                        sc.api_call('chat.postMessage', channel='D9J7J4GJG', text='hi',
                ...
                if not queue.empty():
                    msg = queue.get()
                    sc.api_call('chat.postMessage', channel='D9J7J4GJG', text=msg)
